...
The source build shown here uses [[https://www.sbcl.org/][SBCL]]. This is a portability convenience for
environments where ECL is not readily available, such as some SmartOS zones.
The resulting executable can still manage projects that use other underlying
compilers (see =vend repl=).
...
See [[file:build.lisp][build.lisp]] for how =vend= is built with UIOP. For
application-specific SBCL executables, consider adding:
